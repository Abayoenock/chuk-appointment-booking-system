...
if (@$_GET['t'] == 'year') {
    ...
    while ($k <= $xx) {
        ...
        $sql = "SELECT * FROM `appointments` as a INNER JOIN appointmentapproval as ap on a.appointmentID=ap.appointmentID WHERE  a.appointmentDate>='$start' AND a.appointmentDate <= '$end' AND `appointmentApprove`='0' AND a.departmentID='$departmentID' AND a.doctorID='$userID' ";
        $exe = $conn->query($sql);
        $declined = $exe->num_rows;
        $sql = "SELECT * FROM `appointments` as a INNER JOIN appointmentapproval as ap on a.appointmentID=ap.appointmentID WHERE  a.appointmentDate>='$start' AND a.appointmentDate <= '$end' AND `appointmentApprove`='1' AND a.departmentID='$departmentID' AND a.doctorID='$userID'";
        $exe = $conn->query($sql);
