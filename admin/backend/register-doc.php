...
if ($type == '') {
    ...
    if ($phoneNumber == "") {
        ...
        $sql = "SELECT * FROM `doctors` WHERE email='$email' OR phoneNumber='$phoneNumber'";
        $exe = $conn->query($sql);
        if ($exe->num_rows > 0) {
            $valid['success'] = false;
            ...
            if (in_array($type, array('jpg', 'jpeg', 'png', 'PNG'))) {
                if (is_uploaded_file($_FILES['user_image']['tmp_name'])) {
                    if (move_uploaded_file($_FILES['user_image']['tmp_name'], $url)) {
                        ...
                        $sql2 = "INSERT INTO `doctors`(`doctorID`, `firstname`, `lastname`, `profileImg`, `specialization`, `title`, `departmentID`, `email`, `phoneNumber`, `password`, `status`, `recordedBy`, `dateAddded`) VALUES (null,'$firstName','$lastName','$img1','$specialization','$title','$department','$email','$phoneNumber','$passwordHashed','1','$userID','$tim')";
                        if ($conn->query($sql2) === TRUE) {
                            $valid['success'] = true;
                            $valid['messages'] = "Doctor successfully registered  ";
