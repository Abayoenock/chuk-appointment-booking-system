            ...
            <div class=" w-full md:ml-[250px] bg-white  h-screen overflow-y-auto flex flex-col mainContainer pb-40 p-6 px-10 ">
                ...
                ?>
                    <div class="w-full flex flex-col gap-8 pending-cont">
                        <a href="manage-receptionist?t=add-recp" class="bg-gradient-to-r from-gray-700/70 to-blue-500/70  bg-primary   hover:bg-gradient-to-r hover:from-gray-400/40 hover:to-blue-500/40 transition eas duration-500 rounded-md px-8 py-2 text-gray-100 hover:shadow-xl w-fit   "> <i class="fas fa-user-plus    "></i> Add Receptionist</a>
                ...
                ?>
                    ...
                    <a href="manage-receptionist" class="bg-gradient-to-r from-gray-700/70 to-blue-500/70 bg-primary   hover:bg-gradient-to-r hover:from-gray-400/40 hover:to-blue-500/40 transition eas duration-500 rounded-md px-8 py-2 text-gray-100 hover:shadow-xl w-fit   "> <i class="fas fa-angle-double-left    "></i> View receptionists</a>
                    <form class="mt-8 mb-0 flex  gap-4 text-sm submit font-semibold" action="./backend/register-recp.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-col w-full mt-8">
                            <div class="flex flex-col md:flex-row w-full gap-4 ">
                                <div class="flex flex-col w-full ">
                                    ...
                                    <input type="text" name="firstName" id="fname" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter receprionist's first  name" required>
                                </div>

                                <div class="flex flex-col w-full ">
                                    <label for="lname" class="text-gray-700">LastName</label>
                                    <input type="text" name="lastName" id="lname" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter receprionist's last name" required>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col md:flex-row w-full gap-4 ">
                                <div class="flex flex-col w-full relative">
                                    ...
                                    <input type="email" name="email" id="nid" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter receprionist email  " title="" required>

                                </div>

                                <div class="flex flex-col w-full relative ">
                                    <label for="phoneNumber" class="text-gray-700">Phone Number</label>
                                    <input type="tel" name="phoneNumber" id="phoneNumber" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter receprionist's phone number" pattern="[0-9]{10}" title="Please Rwandan phone number Minus the country code" required>
                            ...
                            <div class="my-4 mb-0 flex items-center justify-end space-x-4">
                                <button class="bg-gradient-to-r from-green-400/70 to-blue-500/70  hover:bg-primary/80   hover:bg-gradient-to-r hover:from-blue-400/70 hover:to-green-500/70 transition eas duration-500 rounded-lg px-8 py-2 text-gray-100 hover:shadow-xl  uppercase submit-btn resister-btn">Create Account &nbsp <i class="fas fa-spinner animate-spin invisible" aria-hidden="true"></i> </button>
                            </div>
                        </div>
                ...
                if (@$_GET['t'] == 'edit-recp') {
                    $recpID = $_GET['recp'];
                    $sql = "SELECT * FROM `receptionist` AS r INNER JOIN department AS dp on r.departmentID=dp.dept_ID WHERE recpID='$recpID' ";
                    $exe = $conn->query($sql);
                    $data = array();
                    while ($row = $exe->fetch_array()) {
                        $data = $row;
                ...
                ?>
                    ...
                    <div class="flex w-full gap-12">
                        <form class="mt-8 mb-0 flex flex-col  gap-4 text-sm submit1 font-semibold" action="./backend/update-img.php?t=receptionist&fname=profileImg&id=<?php echo $recpID ?>&img=<?php echo $profileImg ?>&fid=recpID" method="POST" enctype="multipart/form-data">
                        ...
                        <form class="mt-12 mb-0 flex  gap-4 text-sm submit2 font-semibold w-full" action="./backend/edit-recp.php?id=<?php echo $recpID; ?>" method="POST" enctype="multipart/form-data">
                            <div class="flex flex-col w-full">
                                <div class="flex flex-col md:flex-row w-full gap-4 ">
                                    <div class="flex flex-col w-full ">
                                        <label for="fname" class="text-gray-700">FirstName</label>
                                ...
                                <div class="my-4 mb-0 flex items-center justify-end space-x-4">
                                    <button class="bg-gradient-to-r from-green-400/70 to-blue-500/70  hover:bg-primary/80   hover:bg-gradient-to-r hover:from-blue-400/70 hover:to-green-500/70 transition eas duration-500 rounded-lg px-8 py-2 text-gray-100 hover:shadow-xl  uppercase resister-btn">Update Account &nbsp <i class="fas fa-spinner animate-spin invisible" aria-hidden="true"></i> </button>
                                </div>
                            </div>
    ...
    <script>
        ...
        ?>
            ...
            form.addEventListener("submit", async (event) => {
                ...
                const response = await fetch("./backend/register-recp.php", {
                    method: "POST",
                    body: formData,
                });
        ...
        const loadUsers = () => {
            fetch("./backend/users.php?t=recp")
                .then((resp) => {
                    return resp.json();
                }).then((backData) => {
                        ...
                        columns: [{
...

                            {
                                ...
                                render: (data, type, row, meta) => {
                                    if (row.status == 1) {
                                        return ` <div class="flex gap-2"> <span class="trash-btn px-3 py-2 flex justify-center items-center rounded-md cursor-pointer bg-red-400 text-white" data-id=${row.recpID} ><i class="fa-solid fa-trash-alt text-sm "></i></span>
                                            <span class="edit-btn px-3 py-2 flex justify-center items-center  rounded-md cursor-pointer bg-primary text-white" data-id=${row.recpID} ><i class="fa-solid fa-edit text-sm "></i></span>
                                            
                                            <span class="lock-btn px-3 py-2 flex justify-center items-center  rounded-md cursor-pointer bg-green-400 text-white" data-id=${row.recpID} ><i class="fa-solid fa-house-medical-circle-check text-sm "></i></span>
                                            </div>`
                                    } else {
                                        return ` <div class="flex gap-2"> <span class="trash-btn px-3 py-2 flex justify-center items-center rounded-md cursor-pointer bg-red-400 text-white" data-id=${row.recpID} ><i class="fa-solid fa-trash-alt text-sm "></i></span>
                                            <span class="edit-btn px-3 py-2 flex justify-center items-center  rounded-md cursor-pointer bg-primary text-white" data-id=${row.recpID} ><i class="fa-solid fa-edit text-sm "></i></span>
                                            
                                            <span class="unlock-btn px-3 py-2 flex justify-center items-center  rounded-md cursor-pointer bg-pink-400 text-white" data-id=${row.recpID} ><i class="fa-solid fa-house-medical-circle-xmark text-sm "></i></span>
                                            </div>`
                    ...
                    $(".trash-btn").click(function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                fetch("./backend/delUser.php?&table=receptionist&idName=recpID&id=" + dataId)
                                    .then((resp) => {
                                        return resp.json();
                                    }).then((backData) => {
                                        const gotData = backData;
                    ...
                    $(".lock-btn").click(function() {
                        ...
                        Swal.fire({
                            ...
                            /* Read more about isConfirmed, isDenied below */
                            if (result.isConfirmed) {
                                fetch("./backend/blockUser.php?&table=receptionist&idName=recpID&fieldName=status&status=1&id=" + dataId)
                                    .then((resp) => {
                                        return resp.json();
                                    }).then((backData) => {
                                        const gotData = backData;
                    ...
                    $(".unlock-btn").click(function() {
                        ...
                        Swal.fire({
                            ...
                            /* Read more about isConfirmed, isDenied below */
                            if (result.isConfirmed) {
                                fetch("./backend/blockUser.php?&table=receptionist&idName=recpID&fieldName=status&status=0&id=" + dataId)
                                    .then((resp) => {
                                        return resp.json();
                                    }).then((backData) => {
                                        const gotData = backData;
