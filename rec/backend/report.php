...
if (@$_GET['t'] == 'pending') {
    ...
    $sql = "SELECT * FROM `appointments` as a INNER JOIN patients as p on a.patientID=p.patientID INNER JOIN appointmentapproval as appp on a.appointmentID=appp.appointmentID WHERE a.appointmentStatus='1' AND a.departmentID='$departmentID'  AND appp.appointmentApprove='1' AND a.appointmentDate='$day'";
    $exe = $conn->query($sql);
    while ($row = $exe->fetch_array()) {
        $appointmentID = $row['appointmentID'];
        $patientNames = $row['firstname'] . " " . $row['lastname'];
    ...
    $sql = "SELECT * FROM `appointments` as a INNER JOIN patients as p on a.patientID=p.patientID INNER JOIN appointmentapproval as appp on a.appointmentID=appp.appointmentID WHERE a.appointmentStatus='1' AND a.departmentID='$departmentID' AND appp.appointmentApprove='0' AND a.appointmentDate='$day' ";
    $exe = $conn->query($sql);
    while ($row = $exe->fetch_array()) {
        $appointmentID = $row['appointmentID'];
        $patientNames = $row['firstname'] . " " . $row['lastname'];
