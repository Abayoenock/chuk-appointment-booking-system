...
//$day = strtotime($day);
if (@$_GET['t'] == 'pending') {
    $sql = "SELECT * FROM `appointments` as a INNER JOIN patients as p on a.patientID=p.patientID  WHERE a.appointmentStatus='0' AND a.departmentID='$departmentID' AND a.appointmentDate='$day'";
    $exe = $conn->query($sql);
    while ($row = $exe->fetch_array()) {
    ...
    $sql = "SELECT * FROM `appointments` as a INNER JOIN patients as p on a.patientID=p.patientID INNER JOIN appointmentapproval as appp on a.appointmentID=appp.appointmentID WHERE a.appointmentStatus='1' AND a.departmentID='$departmentID' AND a.appointmentDate='$day' AND appp.appointmentApprove='1'";
    $exe = $conn->query($sql);
    while ($row = $exe->fetch_array()) {
        $appointmentID = $row['appointmentID'];
        $patientNames = $row['firstname'] . " " . $row['lastname'];
    ...
    $sql = "SELECT * FROM `appointments` as a INNER JOIN patients as p on a.patientID=p.patientID INNER JOIN appointmentapproval as appp on a.appointmentID=appp.appointmentID WHERE a.appointmentStatus='1' AND a.departmentID='$departmentID' AND a.appointmentDate='$day'AND appp.appointmentApprove='0' ";
    $exe = $conn->query($sql);
    while ($row = $exe->fetch_array()) {
        $appointmentID = $row['appointmentID'];
        $patientNames = $row['firstname'] . " " . $row['lastname'];
